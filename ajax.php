   ...
   if($m_id!="")
    ...
    else if($date=="" && $page!="")
    {
        if ( strpos($page, '-') !== false ) {
            $sql="SELECT name,page,storetime FROM `fb_fans` WHERE fans_id IN($m_id) AND DATE_FORMAT(`storetime`,'%H:%i:%s') BETWEEN '$time1' and '$time2' and page='$page'";
        } else {
    ...
    {
         if ( strpos($page, '-') !== false ) {
              $sql="SELECT name,page,storetime FROM `fb_fans` WHERE fans_id IN($m_id) AND DATE_FORMAT(`storetime`,'%H:%i:%s') BETWEEN '$time1' and '$time2' and DATE_FORMAT(`storetime`,'%Y/%m/%d')='$date' and page='$page'";
         }
         else
         {
