...
<?php
	...
	if($page!="")
	{
		$sql ="UPDATE class SET `name`='$txt', sort='$sort',page='$page' WHERE id='$id'";
	}
	else
	{
             ...
             <tr>
              ...
              <td>
              	<select name="page">
					...
					while($rowpage = mysql_fetch_assoc($respage))
					{	
						if($row['page']==$rowpage['id'])
						{
							echo '<option selected="selected" value = "'.$rowpage['id'].'">'.$rowpage['id'].'</option>';
						}
						else
						{
							echo '<option value = "'.$rowpage['id'].'">'.$rowpage['id'].'</option>';
						}
