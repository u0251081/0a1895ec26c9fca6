...
<script type="text/javascript">
    function getResult(){
    var number = $('#page').val();
   location.href="home.php?url=p_class&page_id="+number; 
}
	...
	if($pid == "")
	{
        ...
        <center>
            <table class="table table-hover" id="draggable">
                  <select name="page" id="page" class="uniformselect" onchange="getResult(this.value)">
        ...
        <?php
		$sql = "SELECT * FROM class WHERE id = parent_id AND page='$page_id' ORDER by sort";
		$res = mysql_query($sql);
...
<script>
	function add(str)
	{
		if(isNaN(str))
		{
			window.open("p_add.php?page_id=<?php echo $page_id;?>",'',"width=800","height=600");
		}
		else
		{
			window.open("p_add.php?page_id=<?php echo $page_id;?>&pid="+str+"",'',"width=800","height=600");
		}
	}
