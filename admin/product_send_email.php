<?php
function product_send_email($order_no,$product,$order_time,$member_no,$address,$cellphone,$pay_type,$allmoney,$buy_person,$product_id,$p_price_array,$p_qty_array){

$msql="SELECT picture FROM product_img WHERE p_id IN(".implode(',',$product_id).") AND is_main='1'";
...
$sql2="SELECT email,m_name FROM member WHERE member_no='$member_no'";
...
$testpro="";
    for($i=0;$i<count($list_arr);$i++)
    {
        $test=" <tr><td><a href='#m_9036011004944316767_'>
           <img style='margin-right:10px;width:70px;height:70px;float:left;margin-bottom:15px' src='http://www.17mai.com.tw/admin/".$list_arr[$i]['picture']."' alt=''>
            ...
            <div><p style='margin:0 3px 3px'>".$product[$i]."*".$p_qty_array[$i]."</p></div>
          </td>
           <td style='vertical-align:top'>
            <div style='text-align:right;float:right'>
             <span>NT$".$p_price_array[$i]."</span>
             </div>
           </td> </tr>
          ";
  ...
  $to =$email; //收件者
  $subject ="=?UTF-8?B?".base64_encode("[春燕築巢 地方創生電子商城]  #○○訂單編號○○ :".$order_no."  訂單狀態 更新為: 已寄出")."?="; //信件標題
                                ...
                                <table border='0' width='600' cellpadding='0' cellspacing='0' class='m_9036011004944316767container' style='width:600px;max-width:600px'>
                                    <tbody><tr>
                                            <td class='m_9036011004944316767container-padding m_9036011004944316767content' align='left' style='padding-left:24px;padding-right:24px;padding-top:12px;padding-bottom:12px;background-color:#ffffff'>
                                                ...
                                                <br>
                                                <p style='font-weight:bold;font-size:18px;line-height:24px;color:#1c2538'>[春燕築巢 地方創生電子商城] #○○訂單編號○○ :".$order_no." 訂單狀態 更新為: 已寄出 </p>
                                                <p></p><p>訂單狀態為：已完成 </p><p>付款狀態為：已付款 </p><p>送貨狀態為：已寄出</p><p><span style='font-family: arial, sans-serif;'>有關訂單的查詢或要聯絡春燕築巢 地方創生電子商城。</span></p>
                                                ...
                                                <div style='margin-bottom:15px;font-size:14px;color:#555555'><span style='font-family: Helvetica, Arial, sans-serif;'>已寄出</span></div>
                                                ...
                                                <table style='width:100%;font-size:12px;color:#333333'>
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td style='text-align:right;padding-bottom:10px;font-size:12px;color:#555555'>NT$".$allmoney."</td>
                                                        </tr>
