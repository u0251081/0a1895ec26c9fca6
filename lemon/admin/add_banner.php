...
if(isset($_POST['btn']))
{
    if($banner_img != "" && $mobile_banner_img != "")
    {
        ...
        $sql = "INSERT INTO banner SET p_id='$p_id', title='$banner_title', img='".$filedir.$banner_img."', mobile_img='".$filedir2.$mobile_banner_img."',page='$page', add_time='".date('Y-m-d H:i:s')."', `status`='$status'";
        mysql_query($sql);
        ?>
        <script>
            alert('新增成功');
