...
<div class="container">
    <div class="row">
        <table class="table table-bordered table-responsive table-condensed" id="pay_check_div">
            <thead>
            <tr>
                <th colspan="5"><h3 style="font-family: '微軟正黑體'; font-weight: bold; color: #d62408;">Cek pesanan</h3></th>
            </tr>
                ...
                while ($row = mysql_fetch_array($res))
                {
                    if($row['order_over'] != "")
                    {
                        ...
                        <tr id="order_list_<?php echo @$row['aid']; ?>">
                            ...
                            <td width="180">
                                <input type="button" class="btn btn-primary" value="Lihat" onclick="location.href='index.php?page=<?php echo $page;?>&url=order_detail&id=<?php echo $row['aid']; ?>'">
                                <?php
                                if($row['is_effective'] != 3 && $row['is_effective'] != 2)
                                {
                                    ?>
                                    <input type="button" class="btn btn-danger" value="Batal pesanan" name="cancel_btn" o_id="<?php echo $row['aid']; ?>">
                                    <?php
                                }
                                ?>
                            </td>
            ...
            <tr align="right">
                <td colspan="5">
                    <div class="container">
                        <div class="row">
                            <input type="button" class="btn btn-default" value="Kembali ke" onclick="go_back();">&nbsp;&nbsp;
        ...
        if(confirm('是否取消這筆訂單'))
        {
            ...
            ({
                ...
                success:function(i) //成功就....
                {
                    ...
                    {
                       alert('取消成功!');
                        location.href='index.php?page=<?php echo $page;?>&url=order_search';
                    }
                    else
                    {
